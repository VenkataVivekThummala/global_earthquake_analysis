...
# ----------------------------------------------------
# Tab 3: Risk & Depth Analysis
# ----------------------------------------------------
with tab_risk:
    st.subheader("Earthquake Depth and Risk Categorization")
    
    col_r1, col_r2 = st.columns(2)
    
    with col_r1:
        # Risk Categories Pie (Code Cell 26)
        st.markdown("#### Risk Distribution Category Breakdown")
        ...
        fig_risk_pie = px.pie(
            ...
            names='risk', 
            values='count',
            title="Earthquake Risk Categories (Jupyter Cell 26)",
            color='risk',
            color_discrete_map={'Low': '#22c55e', 'Medium': '#eab308', 'High': '#ef4444'}
        )
    ...
    with col_r2:
        # Depth Distribution (Code Cell 20)
        st.markdown("#### Depth Distribution Analysis")
        fig_depth_dist = px.histogram(
            df_filtered, 
            x='depth', 
            nbins=20,
            title="Depth Distribution (Jupyter Cell 20)",
    ...
    # Depth Statistics table (Code Cell 19)
    st.subheader("Depth Descriptive Statistics (Jupyter Cell 19)")
    if not df_filtered.empty:
        depth_stats = df_filtered['depth'].describe().reset_index()
        depth_stats.columns = ['Statistic', 'Value (km)']
        st.table(depth_stats)
    else:
        st.warning("No records to describe.")

# ----------------------------------------------------
# Tab 4: Correlation Analysis
# ----------------------------------------------------
with tab_correlation:
    st.subheader("Earthquake Feature Correlation Analysis")
    
    if not df_filtered.empty:
        # Correlation Heatmap (Code Cell 22)
        corr_cols = ['latitude', 'longitude', 'depth', 'mag']
        ...
        fig_heatmap = px.imshow(
            ...
            zmin=-1, 
            zmax=1,
            title="Earthquake Feature Correlation (Jupyter Cell 22)"
        )
        ...
        st.markdown("#### About Correlation (Jupyter Cell 23)")
        mag_depth_corr = corr_matrix.loc['mag', 'depth']
        lat_lon_corr = corr_matrix.loc['latitude', 'longitude']
        
        st.markdown(f"""
        * **Latitude & Longitude Correlation**: `{lat_lon_corr:.4f}` (minor linear relationship)
        * **Magnitude & Depth Correlation**: `{mag_depth_corr:.4f}` (extremely negligible linear correlation)
        
        **Notebook Conclusion**: *There is no data that is highly correlated with each other. Longitude and latitude are slightly correlated.*
        """)
    else:
        st.warning("No records to compute correlation.")

# ----------------------------------------------------
# Tab 5: Data Explorer
# ----------------------------------------------------
with tab_explorer:
    st.subheader("🔍 Search and Download Processed Dataset")
    st.markdown("Browse, filter, and export the processed earthquake records below.")
    ...
    selected_cols = st.multiselect(
        "Columns to Display",
        options=all_cols,
        default=['time', 'place', 'latitude', 'longitude', 'depth', 'mag', 'risk', 'country']
    )
    
    # Table search box
    search_query = st.text_input("Search locations/countries", "")
    df_table = df_filtered
    if search_query:
        df_table = df_filtered[
            df_filtered['place'].str.contains(search_query, case=False, na=False) |
            df_filtered['country'].str.contains(search_query, case=False, na=False)
        ]
        
    st.write(f"Displaying {len(df_table)} matching records:")
    st.dataframe(df_table[selected_cols], use_container_width=True)
    
    # Download Button
    csv_data = df_table[selected_cols].to_csv(index=False).encode('utf-8')
    st.download_button(
        label="📥 Download Data as CSV",
        data=csv_data,
        file_name="earthquake_analytics_filtered.csv",
        mime="text/csv"
    )
